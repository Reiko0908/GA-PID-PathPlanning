...
class Car:
    ...
    def update(self, omega):
        self.heading = rotate_vect(self.heading, omega + self.noise_omega)
    ...
    def noise(self, frequency):
        if frequency == 0:
            ...
            noise_magnitude = np.random.normal(0, 1)
            self.noise_omega = np.deg2rad(MAX_OMEGA_NOISE*noise_magnitude)
