...
After that, the genes behind the crossover position of the 2 parental chromosomes will be exchanged.
  ...
  where $\eta$ is the maximum value of the the angular velocity noise can ever be achieved. 
    ...
    Firstly, to minimize the computing time, we need to find a good starting point on the curve before performing the Binary Search algorithm by iterating over $N_{bres}$ $t$ values in $\mathbf{t}$ to find the closest point.
    $$$$
